...
void MyGLWidget::modelTransformPatricio2 ()
{
  ...
  /*Le doy la vuelta para que este cabeza para abajo*/
...
//Este view transform es para implementar una escena con angulos de Euler
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e)
{
  ...
  if (zoomEnabled){
  	...
  	FOV += (e->y() - yClick)*0.001;
  	int n = int((FOV/float(M_PI/3.0))*50.0);
  	//emitir el signal con el valor actual del zoom para que el slider se ponga en una pos correcta
    emit zoom(n);
  	projectTransform();
...
/*Funcion para hacer zoom mediante el signal de un slider del designer
  copiar y punto pq npi de lo q hace*/

void MyGLWidget::hagoZoom(int n)
{
  makeCurrent();
...
void MyGLWidget::createBuffersPatricio2 ()
{
  // Carreguem el model de l'OBJ - Atenció! Abans de crear els buffers!
  patr.load("./models/Patricio.obj");
  ...
  GLuint VBO_Patr2[6];
  // Buffer de posicions
  glGenBuffers(6, VBO_Patr2);
  glBindBuffer(GL_ARRAY_BUFFER, VBO_Patr2[0]);
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*patr.faces().size()*3*3, patr.VBO_vertices(), GL_STATIC_DRAW);
  ...
  // Buffer de normals
  glBindBuffer(GL_ARRAY_BUFFER, VBO_Patr2[1]);
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*patr.faces().size()*3*3, patr.VBO_normals(), GL_STATIC_DRAW);
  ...
  // En lloc del color, ara passem tots els paràmetres dels materials
  // Buffer de component ambient
  glBindBuffer(GL_ARRAY_BUFFER, VBO_Patr2[2]);
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*patr.faces().size()*3*3, patr.VBO_matamb(), GL_STATIC_DRAW);
  ...
  // Buffer de component difusa
  glBindBuffer(GL_ARRAY_BUFFER, VBO_Patr2[3]);
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*patr.faces().size()*3*3, patr.VBO_matdiff(), GL_STATIC_DRAW);
  ...
  // Buffer de component especular
  glBindBuffer(GL_ARRAY_BUFFER, VBO_Patr2[4]);
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*patr.faces().size()*3*3, patr.VBO_matspec(), GL_STATIC_DRAW);
  ...
  // Buffer de component shininness
  glBindBuffer(GL_ARRAY_BUFFER, VBO_Patr2[5]);
  glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*patr.faces().size()*3, patr.VBO_matshin(), GL_STATIC_DRAW);
