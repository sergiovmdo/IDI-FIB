...
void MyGLWidget::iniCamera ()
{
  ...
  angleY = 0.0;
  angleX = 0.0;
  FOVini = float(M_PI/3.0);
  FOV = FOVini;
  raw = 1.0;
  OBS = glm::vec3(0.0,0.0,2*radiEsc);
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  TG = glm::translate(TG, glm::vec3(movimentX,-0.5,0));
...
void MyGLWidget::projectTransform ()
{
  ...
  if (perspectiva)
    Proj = glm::perspective(FOV, 1.0f, zNear, zFar);
  else
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_A: { 
      movimentX+=0.5;
      modelTransformPatricio();
      posFocus = glm::vec3(movimentX,-0.5,0.0);
      glUniform3fv(posFocusLoc,1, &posFocus[0]);

      OBS = glm::vec3(movimentX,0.0,0.0);
      viewTransform2();
      break;
    }
    ...
    case Qt::Key_D: { 
      movimentX-=0.5;
      modelTransformPatricio();
      posFocus = glm::vec3(movimentX,-0.5,0);
      glUniform3fv(posFocusLoc,1, &posFocus[0]);
    ...
    case Qt::Key_C: { 
      ...
      if (++camaraPat == 2) camaraPat = 0;

      if (camaraPat == 1){

        OBS = glm::vec3(movimentX,0.0,0.0);
        VRP = glm::vec3(2,0.25,1.0);
        FOV = float(M_PI/2);
        projectTransform2();
        viewTransform2();
      } 
...
{
  glm::mat4 Proj;  // Matriu de projecció
  Proj = glm::perspective(FOV, 1.0f, 0.1f, 3.0f*radiEsc);
  glUniformMatrix4fv (projLoc, 1, GL_FALSE, &Proj[0][0]);
}
...
void MyGLWidget::viewTransform2(){
  ...
  glm::mat4 View;
  View = glm::lookAt(OBS,VRP,glm::vec3(0.0,1.0,0.0));
  glUniformMatrix4fv (viewLoc, 1, GL_FALSE, &View[0][0]);

}
/*
void MyGLWidget::cambioCamara(){
