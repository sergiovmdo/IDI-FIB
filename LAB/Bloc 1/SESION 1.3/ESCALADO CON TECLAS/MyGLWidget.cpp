...
void MyGLWidget::initializeGL ()
{
  ...
  glUniform1f(varLoc,1); //SUPER IMPORTANTE PONERLO
}
...
/*No hay que añadir funciones solo la llamada de la linea 57 y la de la linea 27*/
void MyGLWidget::keyPressEvent (QKeyEvent *e) {
	makeCurrent ();
	switch ( e->key() ) {
		case Qt::Key_S :
		scale += 0.1;
		glUniform1f(varLoc, scale);//Llamamos a la funcion para que se aplique el escalado
		break;
		
		case Qt::Key_D :
		scale -= 0.1;
    glUniform1f(varLoc, scale);
		break;
